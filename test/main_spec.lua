...
describe("World map triggers", function()
    ...
    describe("room entry trigger", function()
        ...
        it("echoes the devices you can work in the room it enters", function()
            stubDiscover(1)
            -- Return device rows only for the devices query; {} for everything else
            -- (fingerprint/exit lookups) so the entry path isn't disturbed.
            helper.mockDbRows = function(sql)
                if string.find(sql, "FROM devices WHERE room_id", 1, true) then
                    return { { id = 4, command = "pull lever",
                               effect = "seal", repeats = "once" } }
                end
                return {}
            end
            helper.simulateLine("You're in the north plaza.")
            assert.is_true(tableContains(helper.echoCalls,
                "[device] #4 pull lever (seal, once)"))
        end)
...
describe("ring-gong-and-fight-in-arena", function()
    ...
    describe("XP check timer", function()
        ...
        it("ntfy notification is throttled to every 90 minutes", function()
            ...
            -- 45 min used to be the window, and no longer is: still silent.
