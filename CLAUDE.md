...
- **Snapshot first, always.** Run `just db-snapshot "<why>"` before any mutating statement. It dumps the live DB to a SQL text file and commits it to the sibling `../tele-arena-db/` repo, so every hand-edit gets a readable, revertable git diff (which rooms/exits/areas changed) instead of an opaque binary `.bak`. Restore a prior state with `sqlite3 tele-arena.db < ../tele-arena-db/tele-arena.sql` (or `git -C ../tele-arena-db show <commit>:tele-arena.sql | sqlite3 <tmp>`). baud holds the DB open (WAL mode); CLI writes are safe, but the user must `/lua reloadScript()` afterward so the in-memory graph picks up the change.
- **Never read the DB with `mode=ro`.** baud keeps it in WAL mode, and a read-only connection (e.g. `sqlite3.connect("file:tele-arena.db?mode=ro", uri=True)`) can't attach the `-shm`/`-wal` files, so it silently returns a *stale* snapshot of the main file — no error, just old data. Room 359 read back as having one exit that way when it really has four, which is the kind of thing you then "fix". Use the `sqlite3` CLI, or a normal read-write Python handle you only SELECT through; both see the WAL.
- **Never reuse one cursor for a loop and a lookup inside it.** `for row in cur.execute(A): ... cur.execute(B)` silently re-points the cursor mid-iteration, so the outer loop ends after its first row with no error. This has twice produced confident, wrong answers about the graph — "0 rooms match this fingerprint", and "town-sewers-165 is a dead end with one exit" when it has two. Use separate `outer`/`inner` cursors, or `.fetchall()` the outer query first.
